...
fn render_title_bar(f: &mut Frame, area: Rect) {
    let title = Paragraph::new(" aptmatic — apt manager")
        .style(Style::default().bg(Color::Blue).fg(Color::White).add_modifier(Modifier::BOLD));
    f.render_widget(title, area);
}

fn render_status_bar(f: &mut Frame, app: &App, area: Rect) {
    let running = app.hosts.iter().filter(|h| {
        matches!(h.status, HostStatus::Connecting | HostStatus::Gathering)
    }).count();
    let hint = " r:refresh  R:refresh all  u:update  U:upgrade  p:purge-rc  t:task output  z:zoom  q:quit";
...
fn host_indicator(h: &crate::app::HostState, tick: u64) -> String {
    match &h.status {
        ...
        HostStatus::Ready => {
            if h.task.as_ref().map(|t| matches!(t.status, TaskStatus::Running)).unwrap_or(false) {
                SPINNER[(tick / 2) as usize % SPINNER.len()].to_string()
            } else {
                "●".to_string()
            }
...
fn render_host_detail(f: &mut Frame, app: &App, host_idx: usize, area: Rect) {
    ...
    lines.push(Line::from(Span::styled(
        format!(" user: {}  port: {}  sudo: {}", h.cfg.user, h.cfg.port, h.cfg.use_sudo),
        ...
        if let Some(latest) = &info.latest_kernel {
            ...
            if pending {
                lines.push(Line::from(vec![
                    ...
                    Span::styled(" ← reboot to activate", Style::default().fg(Color::DarkGray)),
                ]));
            } else {
                lines.push(Line::from(vec![
                    Span::raw("  Latest:  "),
    ...
    if let Some(task) = &h.task {
        ...
        let task_label = match &task.status {
            ...
            TaskStatus::Failed(e) => {
                Span::styled(format!("✗ {}: {e}", task.kind.label()), Style::default().fg(Color::Red))
            }
        };
...
fn render_task_view(f: &mut Frame, app: &App, host_idx: usize) {
    ...
    let header_text = format!("  {} — {}  [{}]", h.cfg.hostname, task.kind.label(), status_str);
